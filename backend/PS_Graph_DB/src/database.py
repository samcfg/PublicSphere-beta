...
class AGEDatabase:
    ...
    def _parse_cypher_results(self, rows: List[Tuple], return_columns: List[str]) -> List[Dict[str, Any]]:
        """Parse AGE query results (shared logic)"""
        results = []
        for row in rows:
            parsed_row = {}
    ...
    def execute_cypher_with_cursor(self, cursor, graph_name: str, cypher_query: str,
                                   return_columns: List[str] = None) -> List[Dict[str, Any]]:
        """Execute Cypher query with provided cursor (no auto-commit)"""
        ...
        return self._parse_cypher_results(rows, return_columns)
